...
create policy if not exists shops_select on public.shops for select using (auth.uid() = user_id);
create policy if not exists shops_insert on public.shops for insert with check (auth.uid() = user_id);
create policy if not exists shops_update on public.shops for update using (auth.uid() = user_id);
create policy if not exists shops_delete on public.shops for delete using (auth.uid() = user_id);

create policy if not exists beans_select on public.bean_batches for select using (auth.uid() = user_id);
create policy if not exists beans_insert on public.bean_batches for insert with check (auth.uid() = user_id);
create policy if not exists beans_update on public.bean_batches for update using (auth.uid() = user_id);
create policy if not exists beans_delete on public.bean_batches for delete using (auth.uid() = user_id);

create policy if not exists bean_origins_select on public.bean_origins for select using (auth.uid() = user_id);
create policy if not exists bean_origins_insert on public.bean_origins for insert with check (auth.uid() = user_id);
create policy if not exists bean_origins_update on public.bean_origins for update using (auth.uid() = user_id);
create policy if not exists bean_origins_delete on public.bean_origins for delete using (auth.uid() = user_id);

create policy if not exists flavor_notes_select on public.flavor_notes for select using (auth.uid() = user_id);
create policy if not exists flavor_notes_insert on public.flavor_notes for insert with check (auth.uid() = user_id);
create policy if not exists flavor_notes_update on public.flavor_notes for update using (auth.uid() = user_id);
create policy if not exists flavor_notes_delete on public.flavor_notes for delete using (auth.uid() = user_id);

create policy if not exists beans_public_select on public.bean_batches for select to anon using (true);
create policy if not exists bean_origins_public_select on public.bean_origins for select to anon using (true);
create policy if not exists flavor_notes_public_select on public.flavor_notes for select to anon using (true);
create policy if not exists shops_public_select on public.shops for select to anon using (true);
